...
@Injectable()
export class UpdateSystemUserPipe implements PipeTransform {
  transform(post: any, metadata: ArgumentMetadata) {
    let { first_name, last_name, password, staff_id } = post;
    ...
    } else if (password && !(password as string).match(passwordRegexp)) {

...
import { getTransactionExportById, createTransactionExport, getProductsCategoriesList, getProductsList, deleteTransactionExportDetails } from "../../api/requests";
...
const CreateUpdateForm = ({ id }: CreateUpdateFormInterface): React.ReactElement => {
  ...
  const handleCategory = (index: number) => async (option: any) => {
    try {
      ...
      setState(prev => {
        ...
        prev.details[index].buy_price = undefined;
  ...
  const handleBuyPrice = (index: number) => (option: any) => {
    setState(prev => {
      prev.details[index].buy_price = option?.value;
      return { ...prev };
    });
  }
  ...
  const confirmDeleteExistsRow = () => {
    deleteTransactionExportDetails(deleteDetail.data.trId, deleteDetail.data.detail_id)
      .then(() => {
  ...
  const goBack = () => {
    ...
    navigate(`/${applicationBaseURL}/${adminPagesPath.transactionExports}${searchStr ? `?${searchStr}` : ''}`);
  }
  ...
  const handleSave = (ev: any) => {
    ...
    setLoading(true);
    setErrors({});
    const send = { details: state.details.map(({ product_id, count, buy_price }) => ({ product_id, count: parseFloat(count as string) || undefined, buy_price })) };
    createTransactionExport(send)
      .then(() => {
            ...
            <Table bordered>
              <thead>
                <tr>
                  ...
                  <th className={'text-center ' + styles.w_200}>{t('products_sell_price')}</th>
              ...
              <tbody>
                {state.details.map((item, i) => {
                  ...
                  const importedPriceLists = selectedProduct?.transaction_imports?.map((t: any) => t.price)?.filter((p: any, i: any, arr: any) => arr.indexOf(p) === i) || [];
                  if (item.buy_price === undefined && importedPriceLists.length === 1) {
                    handleBuyPrice(i)({ value: importedPriceLists[0] });
                  }
                  const row_total_price = selectedProduct?.sell_price * (parseFloat(item.count as string) || 0);
                  ...
                  return (
                    <tr key={k}>
                      ...
                      <td>
                        <FormGroup>
                          {importedPriceLists.length > 1 ? (
                            <>
                              <Select
                                placeholder={''}
                                options={importedPriceLists.map((value: any) => ({ value, label: value }))}
                                value={importedPriceLists.map((value: any) => ({ value, label: value })).find((i: any) => i.value === item.buy_price)}
                                onChange={handleBuyPrice(i)}
                                isDisabled={existsRow}
                                isClearable
                              />
                              <Input type="hidden" invalid />
                              {errors.details?.[i]?.buy_price && <FormFeedback>{t(errors.details?.[i]?.buy_price)}</FormFeedback>}
                            </>
                          ) : (
                            <>
                              <Input
                                type="text"
                                value={formatNumberWithCommas(item.buy_price || '')}
                                invalid={!!errors.details?.[i]?.buy_price}
                                disabled
                              />
                              {errors.details?.[i]?.buy_price && <FormFeedback>{t(errors.details?.[i]?.buy_price)}</FormFeedback>}
                            </>
                      ...
                      <td>
                        <FormGroup>
                          <Input
                            type="text"
                            value={formatNumberWithCommas(selectedProduct?.sell_price || '')}
                            disabled
                          />
                        </FormGroup>
                      </td>
                      ...
                      <td className="text-center">
                        {existsRow ? (
                          <>
                            {checkPermission(profile.data, rightsMapperData.transactionExportDeleteDetails) && (

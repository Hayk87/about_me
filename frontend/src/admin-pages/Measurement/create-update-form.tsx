...
import { getMeasurementById, createMeasurement, updateMeasurement } from "../../api/requests";
...
const CreateUpdateForm = ({ id }: CreateUpdateFormInterface): React.ReactElement => {
  ...
  const updateUnavailable = !!(id && !checkPermission(profile.data, rightsMapperData.measurementUpdate));
  ...
  const goBack = () => {
    ...
    navigate(`/${applicationBaseURL}/${adminPagesPath.measurements}${searchStr ? `?${searchStr}` : ''}`);
  }
  ...
  const handleSave = (ev: any) => {
    ...
    let promise = null;
    if (id) {
      promise = updateMeasurement(id, state);
    } else {
      promise = createMeasurement(state);
    }
    promise
      .then(() => {

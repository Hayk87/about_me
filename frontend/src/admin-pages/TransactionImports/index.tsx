...
import { getTransactionImportsList, deleteTransactionImport, getSystemUsersList } from '../../api/requests';
import { RootState } from "../../store";
import Pagination from "../../components/Pagination";
import Loading from "../../components/Loading";
import SearchForm from "../../components/SearchForm";
import ConfirmDialog from "../../components/Confirm-dialog";
import { initListAndCount } from "../../store/slices/transaction-imports";
import { useToast } from "../../hooks";
import styles from "./styles.module.scss";

export const path: string = adminPagesPath.transactionImports;
export const viewPagePermission: string = rightsMapperData.transactionImportRead;
...
export default function Page() {
  ...
  const rows = useSelector((state: RootState) => state.transactionImports);
  ...
  const addNewItem = () => {
    ...
    navigate(`/${applicationBaseURL}/${adminPagesPath.transactionImportsNew}${searchStr ? `?${searchStr}` : ''}`);
  }

  const editItem = (item: any) => () => {
    const searchStr = queryString.stringify(parsed);
    navigate(`/${applicationBaseURL}/${adminPagesPath.transactionImportsDetailsId.replace(':id', item.id)}${searchStr ? `?${searchStr}` : ''}`);
  }
  ...
  const confirmDeleteDialog = () => {
    deleteTransactionImport(deleteDialog.data?.id)
      .then(() => {
  ...
  const fetchData = () => {
    ...
    getTransactionImportsList(search)
      .then(res => {
    ...
    <>
      <div className={styles.mainContainer}>
        <div className={styles.btns}>
          {checkPermission(profile.data, rightsMapperData.transactionImportCreate) && (
              ...
              {LIST.map(item => (
                <tr key={item.id}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    {checkPermission(profile.data, rightsMapperData.transactionImportReadDetails) && (
                      <>
                    ...
                    {checkPermission(profile.data, rightsMapperData.transactionImportDelete) && (
                      <>
